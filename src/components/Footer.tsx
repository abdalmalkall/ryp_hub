...
const Footer = () => {
  ...
  return (
    <>
      <footer id="contact" className="bg-background text-foreground py-12 md:py-20 font-body border-t border-border" data-reveal>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
            {/* Company Info */}
            <div className="lg:col-span-2" data-reveal style={{ "--reveal-delay": "80ms" } as CSSProperties}>
              <div className="mb-6">
            ...
            {/* Quick Links */}
            <div data-reveal style={{ "--reveal-delay": "140ms" } as CSSProperties}>
              <h3 className="font-display text-lg md:text-xl font-semibold mb-4 md:mb-6 text-foreground">
                Quick Links
              </h3>
                ...
                {quickLinks.map((link) => (
                  link.onClick ? (
                    <button
                      ...
                      className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors w-full text-left text-sm md:text-base hover:translate-x-1 active:scale-95"
                    >
                    ...
                    <a
                      ...
                      className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors text-sm md:text-base hover:translate-x-1 active:scale-95"
                    >
                      {link.external ? (
            ...
            <div data-reveal style={{ "--reveal-delay": "200ms" } as CSSProperties}>
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex items-center gap-3 text-muted-foreground">
                  <div className="w-8 h-8 md:w-10 md:h-10 bg-secondary rounded-lg flex items-center justify-center border border-border">
                    <Mail className="w-4 h-4" />
                  </div>
                  <div>
                    <p className="text-xs md:text-sm text-muted-foreground">Email</p>
                    <a href="mailto:renderyourplan@gmail.com" className="hover:text-foreground transition-colors text-sm md:text-base hover:translate-x-1 active:scale-95 inline-block">
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  ...
                  <div>
                    ...
                    <a href="tel:+962790383135" className="hover:text-foreground transition-colors text-sm md:text-base hover:translate-x-1 active:scale-95 inline-block">
                      +962 7 9038 3135
                    </a>
                  </div>
                </div>
      ...
      {showCv && (
        ...
        >
          <div
            className="relative bg-card text-card-foreground w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-lg border border-border"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-border bg-card">
              <h2 className="font-display text-lg md:text-xl font-bold text-card-foreground">Curriculum Vitae</h2>
              <button
                onClick={() => setShowCv(false)}
                className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-secondary hover:bg-accent flex items-center justify-center transition-colors active:scale-95"
              >
            ...
            {/* Footer Actions */}
            <div className="p-4 border-t border-border bg-secondary/40 flex justify-between items-center">
              <button
                onClick={() => setShowCv(false)}
                className="px-4 md:px-6 py-2 text-muted-foreground hover:text-foreground transition-colors text-sm md:text-base active:scale-95"
              >
                Close
              </button>
